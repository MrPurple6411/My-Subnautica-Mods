...
namespace UnkownName
{
	...
	public class TooltipFactory_WriteIngredients
	{
		...
		public static bool Prefix(ITechData data)
		{
			...
			for (int i = 0; i < ingredientCount; i++)
			{
				...
				if (!KnownTech.Contains(techType) && GameModeUtils.RequiresBlueprints()) return false;
			}
	...
	public class ScannerTool_OnHover
	{
		...
		public static void Postfix(ScannerTool __instance)
		{
			...
			if (__instance.energyMixin.charge <= 0f || !scanTarget.isValid || PDAScanner.CanScan() != PDAScanner.Result.Scan || __instance.stateCurrent == ScannerTool.ScanState.SelfScan || !GameModeUtils.RequiresBlueprints()) return;
