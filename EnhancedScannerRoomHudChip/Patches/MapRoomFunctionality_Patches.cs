...
namespace EnhancedScannerRoomHudChip.Patches
{
...
#if SN1
    using ResourceInfo = ResourceTracker.ResourceInfo; 
#else
    ...
    class MapRoomFunctionality_Patches
    {
        ...
        public static bool StartPrefix(MapRoomFunctionality __instance)
        {
            if (__instance is ScannerChipFunctionality)
            {
                ...
                if(__instance.typeToScan != TechType.None)
                    ...
                    __instance.StartScanning(TechType.Fragment);
...
#else
                ResourceTrackerDatabase.onResourceDiscovered += __instance.OnResourceDiscovered;
                ResourceTrackerDatabase.onResourceRemoved += __instance.OnResourceRemoved;
#endif
        ...
        public static bool OnResourceDiscoveredPrefix(MapRoomFunctionality __instance, ResourceInfo info)
        {
            if (__instance is ScannerChipFunctionality)
            {
        ...
        public static bool OnResourceRemovedPrefix(MapRoomFunctionality __instance, ResourceInfo info)
        {
            if (__instance is ScannerChipFunctionality)
            {
                if (__instance.typeToScan == info.techType)
