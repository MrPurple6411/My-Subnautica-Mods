...
namespace ResourceOverload
{
    ...
    public static class CustomLootDistributionData
    {
        ...
        {
            string path = Config.Randomization
        ...
        private static void AddPrefabToCustomData(LootDistributionData.PrefabData prefabData, BiomeType bio, WorldEntityInfo wei)
        {
            if(!customDSTDistribution[bio].prefabs.Contains(prefabData))
            {
                ...
                string tech0 = Config.Randomization
                    ? TechTypeExtensions.GetOrFallback(Language.main, wei.techType, wei.techType)
                    : TechTypeExtensions.GetOrFallback(Language.main, wei.techType, wei.techType) + "| " + bio.AsString().Split('_')[0];
        ...
        private static void GenerateCustomData(LootDistributionData __instance)
        {
            ...
            string path = Config.Randomization
                ? Path.GetDirectoryName(Assembly.GetAssembly(typeof(Entry)).Location) + "/RandomizerConfig.json"
                : Path.GetDirectoryName(Assembly.GetAssembly(typeof(Entry)).Location) + "/Config.json";
        ...
        public static void LoadSettings()
        {
            ...
            if(File.Exists(path))
            {
                ...
                foreach(string type in techs.Keys)
                {
                    ...
                    {
                        string tech0 = Config.Randomization ? type : type + "| " + bio;
            ...
            foreach(LootDistributionData.PrefabData prefabData in lootDistributionData.dstDistribution[biomeType].prefabs)
            {
                if(WorldEntityDatabase.TryGetInfo(prefabData.classId, out WorldEntityInfo wei))
                {
                    if(wei.techType != TechType.None)
                    {
                        string tech0 = Config.Randomization
                            ? TechTypeExtensions.GetOrFallback(Language.main, wei.techType, wei.techType)
                            : TechTypeExtensions.GetOrFallback(Language.main, wei.techType, wei.techType) + "| " + biomeType.AsString().Split('_')[0];
        ...
        private static void Randomizer(LootDistributionData __instance, BiomeType bio)
        {
            ...
            {
                if(!bio.AsString().Contains("Fragment") && __instance.dstDistribution.TryGetValue(b, out LootDistributionData.DstData d))
                {
